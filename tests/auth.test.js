...
describe('Auth Flow', () => {
    ...
    beforeAll(async () => {
        ...
        await db_1.prisma.permission.upsert({
            where: { name: 'read_user_self' },
            update: {},
            create: { name: 'read_user_self', description: 'Read own user data' },
        });
        const userRole = (await db_1.prisma.role.findUnique({ where: { name: 'CLIENT' } }));
        const readSelfPermission = (await db_1.prisma.permission.findUnique({ where: { name: 'read_user_self' } }));
        ...
        await db_1.prisma.userRole.create({ data: { userId: userId, roleId: userRole.id } });
        // Generate token with roles in payload
        userToken = (0, jwt_1.generateAccessToken)({ userId: userId, roles: [userRole.name] });
    });
    afterAll(async () => {
    ...
    it('should get user profile using access token including roles and permissions', async () => {
        ...
        const permissions = res.body.roles.flatMap((role) => role.role.permissions.map((p) => p.permission.name));
    ...
    it('should deactivate user account', async () => {
        ...
        const inactiveUser = await db_1.prisma.user.findUnique({ where: { id: userId } });
